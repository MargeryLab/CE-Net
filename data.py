...
#色彩饱和度
from utils.repartition_dataset import make_dataset
...
def default_loader(img_path, mask_path):
    ...
    mask = 255. - cv2.resize(mask, (352, 352))
...
def default_DRIVE_loader(img_path, mask_path):
    ...
    mask = np.expand_dims(mask, axis=2)
    img = np.array(img, np.float32).transpose(2, 0, 1) / 255.0 * 3.2 - 1.6
    # mask = np.array(mask, np.float32).transpose(2, 0, 1) / 255
    # mask[mask == 127] = 1
    # mask[mask == 255] = 2
    mask = onehot(mask, 3).transpose(2, 0, 1)
...
def read_DRIVE_datasets(root_path, mode='train'):
    ...
    # else:
    #     image_root = os.path.join(root_path, 'validation/images')
    #     gt_root = os.path.join(root_path, 'validation/masks')
    #
    #
    ...
    if mode is 'train':
        items = make_dataset(root_path, mode)
    elif mode is 'test':
        items = make_dataset(root_path, mode)
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self,root_path, datasets='Messidor',  mode='train'):
        ...
        if self.dataset == 'RIM-ONE':
            ...
            self.items = read_DRIVE_datasets(self.root, self.mode)
    ...
    def __getitem__(self, index):
        img, mask = default_DRIVE_loader(self.items[index][0], self.items[index][1])
        # img = torch.Tensor(img)
        # mask = torch.Tensor(mask)
        # return img, mask, ls[5]
        return img, mask,self.items[index][0].split('/')[-1]
