...
class MyFrame():
    def __init__(self, net, loss=nn.BCELoss, lr=2e-4, evalmode=False):
        self.net = net().cuda()
    ...
    def optimize(self):
        ...
        tumor_dice = diceCoeff(pred[:, 1:2, :], self.mask[:, 1:2, :])
        wall_dice = diceCoeff(pred[:, 2:3, :], self.mask[:, 2:3, :])
        mean_dice = (tumor_dice + wall_dice) / 2

        loss = self.loss(self.mask, pred)
        loss.backward()
        self.optimizer.step()
        return loss.data, mean_dice.data, pred
