...
import resnest.torch.resnest as resnest
...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=Constants.BINARY_CLASS, num_channels=3):
        ...
        # self.decoder4 = DecoderBlock(2052, filters[2])  #256
    ...
    def forward(self, x):
        # Encoder
        x = self.firstconv(x)   #(32,64,128,128)
        x = self.firstbn(x)
        x = self.firstrelu(x)
        x = self.firstmaxpool(x)    #(32,64,64,64)
        e1 = self.encoder1(x)   #64,256
        e2 = self.encoder2(e1)  #128,512
        e3 = self.encoder3(e2)  #256,1024
        e4 = self.encoder4(e3)  #516, 2052
        ...
        # Decoder
        d4 = self.decoder4(e4) + e3 #(32,256,16,16)
        d3 = self.decoder3(d4) + e2 #(32,128,32,32)
        d2 = self.decoder2(d3) + e1 #(32,64,64,64)
        d1 = self.decoder1(d2)  #(32,64,128,128)

        out = self.finaldeconv1(d1) #(32,32,256,256)
        out = self.finalrelu1(out)
        out = self.finalconv2(out)
        out = self.finalrelu2(out)
        out = self.finalconv3(out)  #(32,1,256,256)
